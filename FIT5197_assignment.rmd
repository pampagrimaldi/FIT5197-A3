...
Given the data has two distinctly different distributions, we will need
to complet
...
```{r}
#train test split
split <-  sample.split(df_train1$joyjoy,SplitRatio = 0.9)
training_set1 <-  subset(df_train1,split==TRUE)
testing_set1 <-  subset(df_train1,split==FALSE)
testing_set1X <-  subset(testing_set1,select=-joyjoy)
```
...
pred_svm2 <- predict(regressor, testing_set1X)
```

Note that my previous model scored 2.76 RMSE

```{r}
#RMSE
mean((testing_set1$joyjoy - pred_svm2)^2)
```
...
```{r}
set.seed(123)
ind <- sample(2, nrow(data), replace = T, prob = c(.99, .01))
training <- data[ind==1,1:30]
test <- data[ind==2, 1:30]
trainingtarget <- data[ind==1, 31]
testtarget <- data[ind==2, 31]
str(trainingtarget)
str(testtarget)
```
